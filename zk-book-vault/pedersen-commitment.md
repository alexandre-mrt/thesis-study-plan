...
We assume the reader is already familiar with [[elliptic-curve-addition|elliptic curve point addition]] and scalar multiplication and what it means for a point to be “on the curve.”

Notation-wise, capital letters are elliptic curve points, lowercase letters are finite field elements.

We say $A$ is an elliptic curve (EC) point, $a$ is a [[finite-fields|finite field]] element, and $aA$ is point multiplication between finite field element $a$ and EC point $A$. The expression $A + B$ denotes elliptic curve point addition.
