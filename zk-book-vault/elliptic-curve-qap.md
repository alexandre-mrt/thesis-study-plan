...
Evaluating a [[quadratic-arithmetic-program|Quadratic Arithmetic Program (QAP)]] on a trusted setup enables a prover to demonstrate that a QAP is satisfied without revealing the witness while using a constant sized proof.
...
To make this a little less abstract, let’s say that matrices of the [[rank-1-constraint-system|Rank 1 Constraint System (R1CS)]] $\mathbf{L}$, $\mathbf{R}$, and $\mathbf{O}$ have 3 rows and 4 columns.
...
The solution to this problem is described in the next chapter on the [[groth16|Groth16 protocol]].
