...
[Monero uses Bulletproof Range Proofs](https://github.com/monero-project/monero/blob/master/src/ringct/bulletproofs.cc) (the algorithm presented here) to ensure that the sum of transactions is not negative (in a [[finite-fields|finite field]], the negative numbers are the elements greater than $p/2$ as they are additive inverses of the elements less than or equal $p/2$ where $p$ is the field order).

*This article is part of a series on [ZK Bulletproofs](https://rareskills.io/post/rareskills.io/post/bulletproofs-zk).*
