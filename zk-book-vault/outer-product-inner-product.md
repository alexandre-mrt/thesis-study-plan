...
If we have a [[pedersen-commitment|Pedersen vector commitment]] $A$ which contains a commitment to a vector $\mathbf{a}$ as $A = a_1G_1 + a_2G_2+\dots + a_nG_n$ we can prove we know the opening by sending $\mathbf{a}$ to the verifier who would check that $A \stackrel{?}= a_1G_1 + \dots + a_nG_n$. This requires sending $n$ elements to the verifier (assuming $\mathbf{a}$ is of length $n$).
...
![[off-product-shade-f149ffc5.png|a square matrix with every entry shaded except the main diagonal]]
...
![[pairwise-outer-product-70c52336.png|outer product of pairwise partitions]]
...
](https://r2media.rareskills.io/bulletproofs-06/outerproduct-anim.mp4)
...
![[folded-commitment-49b9c59f.png|a graphic showing the relationship between a vector commitment and a folded vector commitment]]
