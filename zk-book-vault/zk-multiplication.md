...
![[polynomial-multiplication-4c0f0872.png|random-polynomial-multiplication]]

The verifier would then check that $3 \times 4 = 12$ and accept the prover’s claim.

The [[schwartz-zippel-lemma|Schwartz-Zippel lemma]] states that if $f(x) \neq g(x)$ then the probability that $f(u) = g(u)$ for some random value $u$ is less than $d/p$ where $d$ is the maximum degree of the two polynomials and $p$ is the order of the [[finite-fields|finite field]]. If $d \ll p$ ($d$ much much less than $p$), then the probability of $u$ being an intersection point of two non-equal polynomials is negligible.
...
This article is part of a series on [[bulletproofs-zk|Bulletproof ZKPs]].
