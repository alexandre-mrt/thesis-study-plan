...
![[706568_e0befb030c5449aaa5827cfba3fc2c46_mv2-93bc5328.png|Number line ranging from 0 to 2^254 -1 with a separation of p-1. The line of 0 to p-1 is colored green and p-1 to 2^254-1 is colored red.]]

0 to $p - 1$ (the green line segment) is the interval a Circom field element can hold, and p to $2^{254} - 1$ (the red segment) is the values a 254 bit binary value can hold, but a field element cannot.

The “danger zone” is 254 bit binary values larger than `p - 1`. These are the numbers in the interval $[p,  2^{254} − 1]$. In Circom’s (default) case, the range $[p, 2^{254} − 1]$ is
...
To constrain a binary number $b₀, b₁, ..., bₙ$ to equal a field element `v`, we write the following [[arithmetic-circuit]]:
...
## Bug prevention with AliasCheck and Num2Bits\_strict and Bits2Num\_strict
...
![[706568_57d292ca75b946a4aec5cfe7711b8c3e_mv2-b476dab5.jpeg|Template code comparison of Num2bits_stirct() and Bits2Num_strict. highlighting the common uses of AliasCheck template in both templates.]]
...
To inject the wrong values into the signals used to encode the binary array, we need to use the technique describe in our tutorial on [[underconstrained-circom|hacking underconstrained Circom circuits]]. To generate a proof of concept for the code above, we take the following steps.
