...
This article is a chapter in the [[ZK Book|RareSkills Book of Zero Knowledge Proofs]]. It assumes you are familiar with the prior chapters.

## Notation

We refer to an [[elliptic-curves-finite-fields|elliptic curve point]] belonging to the $\mathbb{G}_1$ elliptic curve group as $[x]_1$ and an elliptic curve point belonging to the $\mathbb{G}_2$ elliptic curve group as $[x]_2$. A [[bilinear-pairing|pairing]] between $[x]_1$ and $[x]_2$ is denoted as $[x]_1\bullet[x]_2$ and produces an element in $\mathbb{G}_{12}$. Variables in bold such as $\mathbf{a}$ are vectors, upper case bold letters such as $\mathbf{L}$ are matrices, and field elements (sometimes informally referred to as “scalars”) are lower case letters such as $d$. All arithmetic operations happen in a [[finite-fields|finite field]] with a characteristic that equals the order of the elliptic curve group.

Given an [[arithmetic-circuit|Arithmetic Circuit (ZK Circuit)]], we convert it to a [[rank-1-constraint-system|Rank 1 Constraint System (R1CS)]] $\mathbf{L}\mathbf{a}\circ \mathbf{R}\mathbf{a} = \mathbf{O}\mathbf{a}$ with matrices of dimension $n$ rows and $m$ columns with a witness vector $\mathbf{a}$. Then, we can convert the R1CS to [[quadratic-arithmetic-program|Quadratic Arithmetic Program (QAP)]] by interpolating the columns of the matrices as $y$ values over the $x$ values $[1,2,...,n]$. Since $\mathbf{L}$, $\mathbf{R}$, and $\mathbf{O}$ have $m$ columns, we will end up with three sets of $m$ polynomials:
...
The details of this computation are discussed in our tutorial [[elliptic-curve-qap|Quadratic Arithmetic Programs over Elliptic Curves]].
