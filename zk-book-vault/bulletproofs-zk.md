...
Furthermore, Bulletproofs do not use [[bilinear-pairing|pairings]] (only basic [[elliptic-curve-addition]]) and do not require a [[trusted-setup]].
...
We assume the reader has read and understood the first nine chapters of our [[ZK Book]]. Familiarity with Groth16 or other ZK algorithms is not expected.
...
2. [[pedersen-commitment|Pedersen Commitments]] Pedersen Commitments are what we called the “hash” at the beginning of this article. They are more composable than traditional hash functions, however, as they are additively homomorphic. That is, we can commit 2 to $A$ and 5 to $B$ and “reveal” 7 to $A + B$.
3. [[pedersen-polynomial-commitment|Polynomial Commitments via Pedersen Commitments]] By creating Pedersen commitments of the coefficients of a polynomial, we can prove we 1) committed to a polynomial and 2) evaluated it correctly without revealing the original polynomial.
4. [[zk-multiplication|Zero Knowledge Multiplication]] We can verify that two polynomials were multiplied together correctly by 1) committing them, 2) evaluating them, and 3) checking that the evaluations of the first two multiply to the third. By having a scheme for multiplying polynomials together (with zero knowledge), we also get scalar multiplication for free.
5. [[inner-product-argument|Inner Product Arguments]] Now that we have the mechanism to do zero knowledge proofs for multiplication, it is only a small change to support zero knowledge proofs for the inner product. Specifically, we change the scalar coefficient polynomials to “vector polynomials” and do vector commitments instead of scalar commitments for the coefficients. We also define an operation of “vector polynomial inner product” that enables us to accomplish the inner product argument. While zero knowledge, this argument is not yet succinct.
6. [[outer-product-inner-product|Succinct Inner Product Arguments]] Normally, to prove you correctly computed an inner product with two vectors of length $n$, you would need to send $2n$ elements (i.e. both of the vectors). This chapter shows a clever trick for only sending $n$ elements.
7. [[log-n-vector-commitment-proof|Logarithmic-Sized Proofs of Knowledge for Inner Products]] This chapter uses the algorithm described in chapter 6 recursively to prove we correctly computed an inner product of two vectors of size $n$ with a proof size of only $\mathcal{O}(n)$ data.
8. [[bulletproofs-zkp|Bulletproof ZKP: the algorithm end-to-end]] We combine the zero knowledge inner product argument in chapter 5 with the succint inner product in chapter 7 to produce the Bulletproof. At this point, you have sufficient knowledge to code the algorithm yourself by combining the previous exercises.
9. [[inner-product-algebra]] This chapter introduces and proves various algebraic identities for adding inner products together, which will be useful when learning the Bulletproof range proof.
10. [[random-linear-combination|Random Linear Combinations]] Instead of creating two proofs (or more) for two (or more) inner products, we can create one proof for multiple inner products by using the random linear combination trick.
11. [[range-proof|Range Proofs]] A range proof is proof that a committed value lies in a certain range. Bulletproofs can accomplish this directly, without the need to arithmetize the problem. Bulletproofs range proofs prove that a number can be encoded as the inner product of a vector of powers of 2 $[1,2,4,...,2^{n-1}]$ and a binary vector, but they don’t reveal which bits are one or zero.
